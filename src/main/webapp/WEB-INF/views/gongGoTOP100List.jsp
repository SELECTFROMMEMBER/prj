...
<center>
	...
	<tr>
			...
			<th width="100" style="cursor:pointer" onClick="searchWithSort('readcount desc')"><center>순위</center></th>
			</c:if>
			
			<!-- 만약 파명 "sort" 의 파값이 'reg_date desc' 면 -->
			<!-- 즉 정렬 의지가 'reg_date desc' 면 -->
			<c:if test="${param.sort=='readcount desc'}">
			<th width="100" style="cursor:pointer" onClick="searchWithSort('readcount asc')">순위▼</th>
			</c:if>
			
			<!-- 만약 파명 "sort" 의 파값이 'reg_date asc' 면 -->
			<!-- 즉 정렬 의지가 'reg_date asc' 면 -->
			<c:if test="${param.sort=='readcount asc'}">
			<th width="100" style="cursor:pointer" onClick="searchWithSort('')">순위▲</th>
			</c:if>
	<th>기업</th>
